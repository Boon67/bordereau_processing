...
-- Note: Role names are constructed from DATABASE_NAME
-- Example: BORDEREAU_PROCESSING_PIPELINE_ADMIN, BORDEREAU_PROCESSING_PIPELINE_READWRITE, etc.
...
GRANT USAGE ON SCHEMA &{GOLD_SCHEMA_NAME} TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);
GRANT USAGE ON SCHEMA &{GOLD_SCHEMA_NAME} TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);
GRANT USAGE ON SCHEMA &{GOLD_SCHEMA_NAME} TO ROLE IDENTIFIER($READONLY_ROLE_NAME);

-- Grant permissions on stages
-- ADMIN: Full access
GRANT ALL PRIVILEGES ON STAGE GOLD_STAGE TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);
GRANT ALL PRIVILEGES ON STAGE GOLD_CONFIG TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

-- READWRITE: Read and write access
GRANT READ, WRITE ON STAGE GOLD_STAGE TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);
GRANT READ, WRITE ON STAGE GOLD_CONFIG TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

-- Grant permissions on all existing tables
-- ADMIN: Full privileges
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON ALL VIEWS IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);
...
GRANT SELECT ON ALL VIEWS IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

-- READONLY: Select only
GRANT SELECT ON ALL TABLES IN SCHEMA &{GOLD_SCHEMA_NAME} 
...
-- ADMIN: Full privileges on future objects
GRANT ALL PRIVILEGES ON FUTURE TABLES IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE VIEWS IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE STAGES IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE PROCEDURES IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE TASKS IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

-- READWRITE: Select, Insert, Update, Delete on future tables
GRANT SELECT, INSERT, UPDATE, DELETE ON FUTURE TABLES IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT SELECT ON FUTURE VIEWS IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT USAGE ON FUTURE STAGES IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT USAGE ON FUTURE PROCEDURES IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT OPERATE ON FUTURE TASKS IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

-- READONLY: Select only on future tables and views
GRANT SELECT ON FUTURE TABLES IN SCHEMA &{GOLD_SCHEMA_NAME} 
    TO ROLE IDENTIFIER($READONLY_ROLE_NAME);

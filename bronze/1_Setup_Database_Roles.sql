...
SET GOLD_SCHEMA_FQN = $DATABASE_NAME || '.GOLD';
...
-- Create schemas (in the current database context)
...
CREATE SCHEMA IF NOT EXISTS IDENTIFIER($GOLD_SCHEMA_NAME)
...
-- Set database context for schema grants
USE DATABASE IDENTIFIER($DATABASE_NAME);

-- ADMIN role: Full schema access
GRANT ALL PRIVILEGES ON SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

-- READWRITE role: Usage and create objects
GRANT USAGE ON SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT USAGE ON SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT USAGE ON SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);
...
GRANT ALL PRIVILEGES ON FUTURE TABLES IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE VIEWS IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE STAGES IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE PROCEDURES IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE TASKS IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

-- Same for Silver schema
GRANT ALL PRIVILEGES ON FUTURE TABLES IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE VIEWS IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE STAGES IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE PROCEDURES IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE TASKS IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

-- Same for Gold schema
GRANT ALL PRIVILEGES ON FUTURE TABLES IN SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE VIEWS IN SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE STAGES IN SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE PROCEDURES IN SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

GRANT ALL PRIVILEGES ON FUTURE TASKS IN SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($ADMIN_ROLE_NAME);

-- READWRITE role: Select, Insert, Update, Delete on future tables
GRANT SELECT, INSERT, UPDATE, DELETE ON FUTURE TABLES IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT SELECT ON FUTURE VIEWS IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT USAGE ON FUTURE STAGES IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT USAGE ON FUTURE PROCEDURES IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT OPERATE ON FUTURE TASKS IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

-- Same for Silver schema
GRANT SELECT, INSERT, UPDATE, DELETE ON FUTURE TABLES IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT SELECT ON FUTURE VIEWS IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT USAGE ON FUTURE STAGES IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT USAGE ON FUTURE PROCEDURES IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT OPERATE ON FUTURE TASKS IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

-- Same for Gold schema
GRANT SELECT, INSERT, UPDATE, DELETE ON FUTURE TABLES IN SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT SELECT ON FUTURE VIEWS IN SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT USAGE ON FUTURE STAGES IN SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT USAGE ON FUTURE PROCEDURES IN SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

GRANT OPERATE ON FUTURE TASKS IN SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READWRITE_ROLE_NAME);

-- READONLY role: Select only on future tables and views
GRANT SELECT ON FUTURE TABLES IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READONLY_ROLE_NAME);

GRANT SELECT ON FUTURE VIEWS IN SCHEMA IDENTIFIER($BRONZE_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READONLY_ROLE_NAME);

GRANT SELECT ON FUTURE TABLES IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READONLY_ROLE_NAME);

GRANT SELECT ON FUTURE VIEWS IN SCHEMA IDENTIFIER($SILVER_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READONLY_ROLE_NAME);

GRANT SELECT ON FUTURE TABLES IN SCHEMA IDENTIFIER($GOLD_SCHEMA_NAME) 
    TO ROLE IDENTIFIER($READONLY_ROLE_NAME);
